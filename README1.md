...
- **Comments are synthetic, not real user data**  
  The user annotations (tags and comments) are generated synthetically for testing purposes. While this allows us to simulate user engagement, the lack of real user input means we are not capturing authentic language, perspectives, or behavior patterns that would naturally emerge in a live system.

- **Ingestion from only one data source (Europeana)**  
  The system currently ingests cultural metadata from a single source: the Europeana API. This limits the diversity of collections and institutions represented, and restricts the range of queries and analysis we can support. Integration with additional data providers would increase both richness and representativeness.

- **Metadata may contain missing or incomplete fields**  
  Many records from Europeana include incomplete metadata, such as missing image URLs, undefined creators, or vague descriptions. This degrades the quality of downstream processes like embedding extraction, content-based recommendations, and semantic enrichment.

- **Europeana API does not support true scrolling**  
  Although the API provides cursors for pagination, it lacks robust support for deep scrolling or iterating over entire datasets across large timeframes. This makes it difficult to ensure full data coverage, especially when trying to download thousands of records grouped by specific criteria.
...
- **Integrate additional open data sources**  
  Adding support for additional cultural or institutional data sources (e.g., Wikidata or national archives) would expand the system’s applicability. It would also enable more complex queries, such as linking artifacts across collections and enriching metadata through cross-referencing.

- **Use real annotations from external platforms**  
  Instead of relying solely on synthetic annotations, we could harvest or integrate real comments, tags, and descriptions from public platforms such as Wikipedia, Reddit or institutional archives. This would provide richer user-generated content and enable more meaningful insights about audience interpretation.

- **Implement Delta Lake OPTIMIZE**  
  Currently, repeated insertions and merge operations into Delta Tables result in many small files, degrading performance over time. Implementing the `OPTIMIZE` command in Delta Lake would compact these files, reduce I/O costs, and improve overall processing efficiency for batch queries and joins.

- **Enable temporal analysis of artifacts and annotations**  
  By grouping cultural objects and annotations by creation or annotation timestamp, we could support historical and behavioral trend analysis. For example, we could identify peaks in user activity or observe patterns in artifact production across centuries, which could support research in digital humanities and cultural studies.
